...
def compute_sift_keypoints1(filename):
    """
...
def compute_orb_keypoints2(filename):
    """
    Reads image from filename and computes ORB keypoints
    Returns image, keypoints and descriptors. 
    """
...
def compute_sift_keypoints3(filename_rgb,filename_dep):
    """
    Reads image from filename and computes ORB keypoints
    Returns image, keypoints and descriptors. 
    """
    ...
    img_rgb = cv2.resize(img_rgb,(1280,640),interpolation=cv2.INTER_LINEAR)
    depth_map = cv2.imread(filename_dep, cv2.IMREAD_GRAYSCALE)
    depth_map = cv2.resize(depth_map,(1280,640),interpolation=cv2.INTER_LINEAR)
    ...
    # Set SIFT parameters
    sift2.setContrastThreshold(contrast_threshold)
    sift2.setEdgeThreshold(edge_threshold)

    # Detect keypoints and compute descriptors for RGB image
    keypoints_rgb, descriptors_rgb = sift1.detectAndCompute(grayscale_image, None)
    ...
    keypoints_combined = np.concatenate((keypoints_rgb, keypoints_depth), axis=0)
    ...
    # resized_depth_map = cv2.resize(depth_map, (grayscale_image.shape[1], grayscale_image.shape[0]))
...
def compute_fundamental_matrix(filename1_rgb,filename1_dep, filename2_rgb,filename2_dep,mode=None):
    ...
    if mode == 'sift':
        img1, kp1, des1 = compute_sift_keypoints1(filename1_rgb)
        img2, kp2, des2 = compute_sift_keypoints1(filename2_rgb)
        matches = brute_force_matcher(des1, des2)
        thres = 500
    elif mode == 'orb':
        img1, kp1, des1 = compute_orb_keypoints2(filename1_rgb)
        img2, kp2, des2 = compute_orb_keypoints2(filename2_rgb)
        matches = brute_force_matcher2(des1, des2)
        thres = 30
    else:
        img1, kp1, des1 = compute_sift_keypoints3(filename1_rgb, filename1_dep)
        img2, kp2, des2 = compute_sift_keypoints3(filename2_rgb, filename2_dep)
        matches = brute_force_matcher(des1, des2)
        thres = 500
    ...
    F, mask = cv2.findFundamentalMat(pts1,pts2,cv2.FM_RANSAC, ransacReprojThreshold=2, confidence=0.99)
...
def evaluate(gt_txt,mode=None):
    ...
    for line in tqdm(fp):
        ...
        if mode == 'sift':
            match_points.append(compute_fundamental_matrix(query, '' , reference, '' , 'sift'))
        elif mode == 'sift_dep':
            match_points.append(compute_fundamental_matrix(query,'./data_dep/'+query, reference , './data_dep/'+reference, 'sift_dep'))
        else:
            match_points.append(compute_fundamental_matrix(query, '' , reference, '' , 'orb'))
        labels.append(gt)
